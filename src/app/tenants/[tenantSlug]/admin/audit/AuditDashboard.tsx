...
export default function AuditDashboard() {
    ...
    return (
        <UniversalLayoutWrapper profileRole="admin" profileName="">
                        ...
                        >
                            {filteredSystemLogs.map((log, idx) => (
                                <motion.div
                                    ...
                                    className={`group bg-[#1c1c1e] border rounded-3xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:scale-[1.015] hover:-translate-y-0.5 transition-all duration-300 ${
                                        log.operacion === 'INSERT' ? 'border-emerald-500/10 hover:border-emerald-500/30 hover:shadow-[0_0_15px_rgba(16,185,129,0.08)]' :
                                        log.operacion === 'DELETE' ? 'border-red-500/10 hover:border-red-500/30 hover:shadow-[0_0_15px_rgba(239,68,68,0.08)]' :
                                        'border-blue-500/10 hover:border-blue-500/30 hover:shadow-[0_0_15px_rgba(59,130,246,0.08)]'
                                    }`}
                                >
                                    <div className="flex items-center gap-6">
...
function RenderDiff({ oldData, newData }: { oldData: any; newData: any }) {
    ...
    return (
        <div className="space-y-4 font-mono text-xs max-h-[40vh] overflow-y-auto bg-[#050505] p-6 rounded-3xl border border-white/5 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
            {allKeys.map((key) => {
                ...
                if (!hasChanged) {
                    return (
                        <div key={key} className="text-gray-500 py-2 border-b border-white/[0.02] flex items-start justify-between gap-4">
                            <span className="text-gray-600 shrink-0 select-none">{key}:</span>
                            <span className="text-gray-400 break-all text-right max-w-md whitespace-pre-wrap font-mono leading-relaxed bg-white/[0.01] px-2 py-0.5 rounded border border-white/[0.02]">
                ...
                return (
                    <div key={key} className="space-y-2 py-3 border-b border-white/5">
                        <div className="text-purple-400/90 font-black tracking-tight text-[11px] uppercase select-none">{key}</div>
                        {oldVal !== undefined && (
                            <div className="bg-red-500/[0.06] text-red-400 px-4 py-3 rounded-2xl border border-red-500/10 flex items-start gap-3 shadow-[inset_0_1px_0_rgba(239,68,68,0.05)]">
                                <span className="text-red-500 font-black shrink-0 select-none">-</span>
                                <span className="break-all whitespace-pre-wrap font-mono leading-relaxed text-red-300/90">
                                    {typeof oldVal === 'object' ? JSON.stringify(oldVal, null, 2) : String(oldVal)}
                                </span>
                            </div>
                        )}
                        {newVal !== undefined && (
                            <div className="bg-emerald-500/[0.06] text-emerald-400 px-4 py-3 rounded-2xl border border-emerald-500/10 flex items-start gap-3 shadow-[inset_0_1px_0_rgba(16,185,129,0.05)]">
                                <span className="text-emerald-500 font-black shrink-0 select-none">+</span>
                                <span className="break-all whitespace-pre-wrap font-mono leading-relaxed text-emerald-300/90">
                                    {typeof newVal === 'object' ? JSON.stringify(newVal, null, 2) : String(newVal)}
                                </span>
                            </div>
                        )}
